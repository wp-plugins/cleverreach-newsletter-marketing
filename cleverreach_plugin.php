...
Description: Widget f&uuml;r die Integration eines CleverReach Anmeldeformulars in der Sidebar und Plugin für die Abonnentengewinnung über Kommentare.
...
$cr_plugin_url = get_option('siteurl').'/wp-content/plugins/cleverreach-newsletter-marketing/';
...
function cr_options() {
	...
	global $cr_plugin_url, $cr_prefix, $api, $domain_url;
	...
	<form id="cr_form" name="cr_form" enctype="application/x-www-form-urlencoded"  method="post" action="<?php str_replace( '%7E', '~', $_SERVER['REQUEST_URI']); ?>">
		...
		<table width="100%" class="form-table">
			...
			<tr>
			  ...
			  <td colspan="2" valign="top"><?php if(!$api_key_ok){ ?><?php if($cr_options["api_key"] != "") { ?><span style='font-weight:bold; color:#cc2222'>API Key nicht korrekt.</span><br /><?php } ?>

              <small>Den API Key finden Sie in Ihrem CleverReach Kundencenter unter Account-&gt;API</small><?php } ?></td>
        ...
        <table width="100%" class="form-table">
			...
			<select name="list_id" id="list_id" style="width:300px;" onchange="loadForms('<?php _e($domain_url); ?>')">
  ...
  if ( !function_exists('register_sidebar_widget') )
	...
	function cr_widget_options() {
		...
		<?php if(!$ca->login()){ $cr_display = "block"?>
			...
			<a href="options-general.php?page=cleverreach-newsletter-marketing/cleverreach_plugin.php">CleverReach Plugin konfigurieren</a>
